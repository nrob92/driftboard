  ...
  try {
    ...
    if (type === "photo") {
      ...
      if (sourcePhoto.session_id !== sessionId) {
        return NextResponse.json(
          { error: "Source photo is not in this session" },
          { status: 400 },
        );
      }
      ...
      const {
        data: { publicUrl: newPhotoUrl },
      } = supabase.storage.from("collab-photos").getPublicUrl(storagePath);
    ...
    if (type === "folder") {
      ...
      if (sourceFolder.session_id !== sessionId) {
        return NextResponse.json(
          { error: "Source folder is not in this session" },
          { status: 400 },
        );
      }
