...
export async function POST(request: NextRequest) {
  try {
    ...
    if (storagePath) {
      ...
      const { error: photosErr } = await supabase.storage
        .from(bucket)
        .remove([storagePath]);
      if (photosErr) {
        ...
        return NextResponse.json(
          {
            error: `Failed to delete from ${bucket}`,
            details: photosErr.message,
          },
          { status: 500 },
        );
      }
      ...
      const fileName =
        lastSlash < 0 ? storagePath : storagePath.slice(lastSlash + 1);
