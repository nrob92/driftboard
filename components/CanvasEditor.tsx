...
function getEditSnapshot(img: CanvasImage): Partial<CanvasImage> {
  ...
  for (const key of EDIT_KEYS) {
    ...
    (out as Record<string, unknown>)[key] = cloneEditValue(
      key as keyof CanvasImage,
      v,
    );
  }
  return out;
...
export function CanvasEditor({
  ...
  const [folderLabelWidths, setFolderLabelWidths] = useState<
  ...
  const sliderSettledWhileDragging = useInteractionStore(
    (s) => s.sliderSettledWhileDragging,
  );
  ...
  const setSliderSettledWhileDragging =
    intActions.setSliderSettledWhileDragging;
  const setIsSpacePressed = intActions.setIsSpacePressed;
  const setDragHoveredFolderId = intActions.setDragHoveredFolderId;
  const setDragSourceFolderBorderHovered =
    intActions.setDragSourceFolderBorderHovered;
  ...
  const selectedExistingFolderId = useUIStore(
    (s) => s.selectedExistingFolderId,
  );
  const folderNameError = useUIStore((s) => s.folderNameError);
  const createFolderFromSelectionIds = useUIStore(
    (s) => s.createFolderFromSelectionIds,
  );
  const createFolderFromSelectionName = useUIStore(
    (s) => s.createFolderFromSelectionName,
  );
  const createFolderFromSelectionNameError = useUIStore(
    (s) => s.createFolderFromSelectionNameError,
  );
  ...
  const createEmptyFolderNameError = useUIStore(
    (s) => s.createEmptyFolderNameError,
  );
  ...
  const createSocialLayoutNameError = useUIStore(
    (s) => s.createSocialLayoutNameError,
  );
  ...
  const deleteFolderDontAskAgain = useUIStore(
    (s) => s.deleteFolderDontAskAgain,
  );
  ...
  const applyPresetToSelectionIds = useUIStore(
    (s) => s.applyPresetToSelectionIds,
  );
  ...
  const setCreateFolderFromSelectionIds =
    uiActions.setCreateFolderFromSelectionIds;
  const setCreateFolderFromSelectionName =
    uiActions.setCreateFolderFromSelectionName;
  const setCreateFolderFromSelectionNameError =
    uiActions.setCreateFolderFromSelectionNameError;
  ...
  const setCreateSocialLayoutNameError =
    uiActions.setCreateSocialLayoutNameError;
  ...
  const lastSwappedImageRef = useRef<{
    id: string;
    x: number;
    y: number;
  } | null>(null);
  const dragPrevCellRef = useRef<{
    imageId: string;
    col: number;
    row: number;
    cellIndex: number;
  ...
  const preZoomViewRef = useRef<{ scale: number; x: number; y: number } | null>(
    null,
  );
  ...
  const multiSelectSaveTimeoutRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const {
    ...
    handleExport,
    handleExportSelection: handleExportSelectionBase,
    handleExportLayout,
  } = useExport({ images, selectedIds, folders, decodeDNG });

  // Bridge: saveStatus/exportProgress from hooks to uiStore (hooks own internal state)
  const uiStoreSync = useUIStore.getState();
  useEffect(() => {
    uiStoreSync.setSaveStatus(saveStatus);
  }, [saveStatus, uiStoreSync]);
  useEffect(() => {
    uiStoreSync.setExportProgress(exportProgress);
  }, [exportProgress, uiStoreSync]);
  ...
  const { data: photoData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [editsResult, foldersResult, photosResult] = await Promise.all([
        ...
        supabase.storage.from(bucket).list(storagePath, {
          limit: 500,
  ...
  useEffect(() => {
    const loadUserPhotos = async () => {
      ...
      const isImageFile = (name: string) =>
        /\.(jpe?g|png|webp|dng|tiff?)$/i.test(name);
      const photosList = (photosFiles ?? []).filter(
        (f) => !f.name.startsWith(".") && isImageFile(f.name),
      );
      ...
      try {
        ...
        const filteredPhotosList = photosList.filter(
          (f) => !f.name.toLowerCase().endsWith(".dng"),
        );
        ...
        const loadBatch = async (start: number, end: number) => {
          ...
          const batchItems = batchFiles.map((f) => ({
            bucket: f.bucket,
            path: `${storageBase}/${f.name}`,
          }));
        ...
        const newImages =
          savedEdits && savedEdits.length > 0
            ? loadedImages.map((img) => {
                const edit = savedEdits.find(
                  (e: PhotoEdits) =>
                    ...
                    (e.original_storage_path != null &&
                      e.original_storage_path === img.storagePath),
                );
                if (!edit) return img;
                const hasValidPosition =
                  edit.x != null &&
                ...
                return {
                  ...
                  ...(edit.original_storage_path != null && {
                    originalStoragePath: edit.original_storage_path,
                  }),
                  width: savedWidth,
                  height: savedHeight,
                  folderId:
                  ...
                  ...(edit.camera_make != null && {
                    cameraMake: edit.camera_make,
                  }),
                  ...(edit.camera_model != null && {
                    cameraModel: edit.camera_model,
                  }),
                  ...(edit.labels != null &&
                    Array.isArray(edit.labels) && { labels: edit.labels }),
  ...
  const visibleImageIds = useMemo(() => {
    ...
    for (const img of images) {
      if (term && !img.labels?.some((l) => l.toLowerCase().includes(term)))
        continue;
      if (
        photoFilter.dateFrom &&
        img.takenAt &&
        img.takenAt < photoFilter.dateFrom
      )
        continue;
      if (photoFilter.dateTo && img.takenAt && img.takenAt > photoFilter.dateTo)
        continue;
      if (photoFilter.cameraMake && img.cameraMake !== photoFilter.cameraMake)
        continue;
      if (
        photoFilter.cameraModel &&
        img.cameraModel !== photoFilter.cameraModel
      )
        continue;
      set.add(img.id);
    }
  ...
  const viewportVisibleIds = useViewportCulling(
    images,
    stagePosition,
    stageScale,
    dimensions.width,
  ...
  const resolveOverlapsAndReflow = useCallback(
    (
      ...
      const resolvedFolders = resolveFolderOverlaps(
        currentFolders,
        currentImages,
        changedFolderId,
      );
      ...
      for (let i = 0; i < resolvedFolders.length; i++) {
        ...
        if (
          !oldFolder ||
          (oldFolder.x === newFolder.x && oldFolder.y === newFolder.y)
        )
          continue;
        ...
        const isChangedFolderWithAdd =
          newFolder.id === changedFolderId && addedImageId;
        ...
        if (existingImgs.length > 0) {
          const oldHeight =
            oldFolder.height ??
            getFolderBorderHeight(oldFolder, oldFolder.imageIds.length);
          const newHeight =
            newFolder.height ??
            getFolderBorderHeight(newFolder, newFolder.imageIds.length);
          const movedOnly =
            oldFolder.width === newFolder.width && oldHeight === newHeight;
          const shouldKeepPositions =
            movedOnly && !isChangedFolderWithAdd && !isSocialLayout(newFolder);

          const reflowed = shouldKeepPositions
            ? existingImgs.map((img) => ({
                ...img,
                x: img.x + dx(newFolder, oldFolder),
                y: img.y + dy(newFolder, oldFolder),
              }))
          ...
          updatedImages = updatedImages.map(
          ...
          if (addedImg) {
            updatedImages = updatedImages.map((img) =>
              img.id === addedImageId
                ? {
                    ...img,
                    x: addedImg.x + dx(newFolder, oldFolder),
                    y: addedImg.y + dy(newFolder, oldFolder),
                  }
                : img,
            );
          }
  ...
  const handleUndo = useCallback(() => {
    ...
    if (currentImage) {
      setEditRedoStack((prev) => [
        ...prev,
        { imageId: entry.imageId, snapshot: getEditSnapshot(currentImage) },
      ]);
    }
    setEditHistory((prev) => prev.slice(0, -1));
    setImages((prev) =>
      prev.map((i) =>
        i.id === entry.imageId ? { ...i, ...entry.snapshot } : i,
      ),
    );
  }, [editHistory, images]);
      ...
      if (touch1 && touch2) {
        ...
        if (lastTouchDistance !== null && lastTouchCenter !== null) {
          ...
          const newScale = Math.max(
            0.1,
            Math.min(20, stageScale * scaleChange),
          );
        ...
        const newPos = {
          x:
            touch1.clientX -
            stageBox.left -
            (lastTouchCenter?.x || touch1.clientX - stageBox.left - stage.x()),
          y:
            touch1.clientY -
            stageBox.top -
            (lastTouchCenter?.y || touch1.clientY - stageBox.top - stage.y()),
        };
  ...
  useEffect(() => {
    if (selectedIds.length > 1)
      lastMultiSelectionRef.current = selectedIds.slice();
  ...
  const handleObjectClick = useCallback(
    (e: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      if (shift) {
        ...
        const lastImg =
          lastId != null ? images.find((img) => img.id === lastId) : null;

        if (clickedImg && lastImg) {
          const folderFor = (img: CanvasImage) =>
            folders.find((f) => f.imageIds.includes(img.id));
        ...
        const lastImageIndex =
          lastId != null ? images.findIndex((img) => img.id === lastId) : -1;
      ...
      const multi =
        selectedIds.length > 1
          ...
          : lastMultiSelectionRef.current &&
              lastMultiSelectionRef.current.includes(imageId)
  ...
  const handleImageTouchStart = useCallback(
    (_e: Konva.KonvaEventObject<TouchEvent>, imageId: string) => {
  ...
  const handleImageTouchMove = useCallback(
    (_e: Konva.KonvaEventObject<TouchEvent>, _imageId: string) => {
      const touch = _e.evt.touches[0];
  ...
  const handleCopyEdit = useCallback(() => {
    ...
    if (img) {
      ...
      setCopiedEdit(
        JSON.parse(JSON.stringify(snapshot)) as Partial<CanvasImage>,
      );
    }
  ...
  const handlePasteEdit = useCallback(async () => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      setImages((prev) =>
        prev.map((img) =>
          doneIds.has(img.id) ? { ...img, ...copiedEdit } : img,
        ),
      );
  ...
  const handleCreatePresetSave = useCallback(async () => {
    ...
    const settings = JSON.parse(
      JSON.stringify(getEditSnapshot(img)),
  ...
  const handleApplyPresetToSelection = useCallback(
    async (preset: Preset) => {
      ...
      useUIStore
        .getState()
        .setApplyPresetProgress({ current: 1, total: ids.length });
      ...
      for (let i = 0; i < total; i++) {
        ...
        setImages((prev) =>
          prev.map((img) =>
            doneIds.has(img.id) ? { ...img, ...resetSettings } : img,
          ),
        );
        await new Promise((r) => setTimeout(r, 16));
      }
    ...
    [
      applyPresetToSelectionIds,
      setApplyPresetToSelectionIds,
      saveToHistory,
      handleSave,
    ],
  );
  ...
  const handleCreateSocialLayoutSave = useCallback(async () => {
    ...
    const pages = Math.max(
      1,
      Math.min(SOCIAL_LAYOUT_MAX_PAGES, createSocialLayoutPages),
    );
    const centerX = (dimensions.width / 2 - stagePosition.x) / stageScale;
    const centerY = (dimensions.height / 2 - stagePosition.y) / stageScale;
    const folderId = `folder-${Date.now()}`;
    ...
    (folderId: string, color: string) => {
      ...
      if (colorPreviewRafRef.current == null) {
        colorPreviewRafRef.current = requestAnimationFrame(() => {
          ...
          if (p) {
            const { folders: currentFolders, setFolders } =
  ...
  useEffect(() => {
    ...
    const close = (e?: MouseEvent) => {
      if (e?.target && imageContextMenuRef.current?.contains(e.target as Node))
        return;
      if (e?.target && canvasContextMenuRef.current?.contains(e.target as Node))
        return;
      if (e?.target && folderContextMenuRef.current?.contains(e.target as Node))
        return;
      if (e?.target && borderDialogRef.current?.contains(e.target as Node))
        return;
      setImageContextMenu(null);
      ...
      dragMoveRafRef.current = requestAnimationFrame(() => {
        ...
        if (currentImg.folderId) {
          const sourceFolder = folders.find(
            (f) => f.id === currentImg.folderId,
          );
          if (sourceFolder) {
            const folderImageCount = images.filter((img) =>
              sourceFolder.imageIds.includes(img.id),
            ).length;
            const borderH = getFolderBorderHeight(
              sourceFolder,
              folderImageCount,
            );
            const left = sourceFolder.x;
            const top = sourceFolder.y + 30;
            const dist = distanceToRectBorder(
              currentCenterX,
              currentCenterY,
              left,
              top,
            ...
            setDragSourceFolderBorderHovered(
              dist <= BORDER_BLINK_THRESHOLD ? currentImg.folderId : null,
            );
          } else {
        ...
        let targetFolder: PhotoFolder | undefined = folders.find(
          (f) => f.id === currentImg.folderId,
        );
        ...
        for (const folder of folders) {
          ...
          if (
            currentCenterX >= bounds.x &&
            currentCenterX <= bounds.right &&
            currentCenterY >= boundTop &&
            currentCenterY <= boundBottom
          ) {
        ...
        if (targetFolderId && targetFolder) {
          ...
          const targetCellCenterX =
            contentStartX + clampedCol * CELL_SIZE + imageMaxSize / 2;
          const targetCellCenterY =
            contentStartY +
            targetRow * CELL_HEIGHT +
            GRID_CONFIG.imageMaxHeight / 2;
          ...
          const otherFolderImages = images.filter(
            (img) =>
              targetFolder!.imageIds.includes(img.id) &&
          ...
          if (
            !dragPrevCellRef.current ||
            dragPrevCellRef.current.imageId !== currentImg.id
          ) {
            const currentImgRelativeX = currentImg.x - contentStartX;
            const currentImgRelativeY = currentImg.y - contentStartY;
            const currentImgCol = Math.max(
              0,
              Math.min(cols - 1, Math.floor(currentImgRelativeX / CELL_SIZE)),
            );
            const currentImgRow = Math.max(
              0,
              Math.floor(currentImgRelativeY / CELL_HEIGHT),
            );
  ...
  const handleObjectDragEnd = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>, type: "image" | "text") => {
      ...
      if (type === "image") {
        ...
        if (currentImg) {
          ...
          for (const folder of latestFolders) {
            const folderHeight =
              folder.height ??
              getFolderBorderHeight(folder, folder.imageIds.length);
            ...
            if (
              currentCenterX >= boundLeft &&
              currentCenterX <= boundRight &&
              currentCenterY >= boundTop &&
              currentCenterY <= boundBottom
            ) {
          ...
          if (targetFolderId && targetFolder) {
            if (isSocialLayout(targetFolder)) {
              ...
              const { folderPadding, imageMaxSize, imageMaxHeight } =
                GRID_CONFIG;
              const cols = calculateColsFromWidth(targetFolder.width);
              const contentStartX = targetFolder.x + folderPadding;
              const contentStartY = targetFolder.y + 30 + folderPadding;
              const folderHeight =
                ...
                getFolderBorderHeight(
                  targetFolder,
                  targetFolder.imageIds.length,
                );
              const contentHeight = folderHeight - 2 * folderPadding;
              const maxRows = Math.max(
                1,
                Math.floor(
                  (contentHeight + GRID_CONFIG.imageGap) / CELL_HEIGHT,
                ),
              );
              const relativeX = currentX - contentStartX;
              const relativeY = currentY - contentStartY;
              const targetCol = Math.max(
                0,
                Math.min(cols - 1, Math.round(relativeX / CELL_SIZE)),
              );
              const targetRow = Math.max(
                0,
                Math.min(maxRows - 1, Math.round(relativeY / CELL_HEIGHT)),
              );
          ...
          // If image is outside folders, use snapping logic
            ...
            const nearest = findNearestPhoto(
              currentCenterX,
              currentCenterY,
              latestImages,
              node.id(),
          ...
          if (targetFolderId !== oldFolderId) {
            ...
            if (!targetFolderId && oldFolderId) {
              ...
              const finalImg = resolvedImages.find(
              ...
              if (user) {
                const resolvedNewFolder = resolvedFolders.find(
                  (f) => f.id === newFolderId,
                );
                const newF = resolvedNewFolder || {
                  x: newFolderX,
                  y: newFolderY,
                };
            ...
            if (targetFolderId) {
              ...
              if (
                ...
                const targetCol = Math.max(
                  0,
                  Math.min(cols - 1, Math.floor(relativeX / CELL_SIZE)),
                );
                const targetRow = Math.max(
                  0,
                  Math.min(maxRows - 1, Math.floor(relativeY / CELL_HEIGHT)),
                );
                const fitScale = Math.min(
                  imageMaxSize / finalWidth,
                  imageMaxHeight / finalHeight,
                  1,
                );
              ...
              const updatedFolders = latestFolders.map((f) => {
                if (f.id === oldFolderId) {
                  return {
                    ...f,
                    imageIds: f.imageIds.filter((id) => id !== currentImg.id),
                  };
                }
              ...
              if (user) {
                ...
                if (lastSwappedImageRef.current) {
                  const swappedImg = resolvedImages.find(
          ...
          if (targetFolderId && targetFolderId === oldFolderId) {
            ...
            if (!isSocialLayout(targetFolder)) {
              ...
              const otherFolderImages = latestImages.filter(
                (img) =>
                  targetFolder!.imageIds.includes(img.id) &&
                  img.id !== currentImg.id,
              );
              ...
              if (occupiedById) {
                ...
                if (
                  prevCell &&
                  prevCell.imageId === currentImg.id &&
                  prevCell.cellIndex !== targetCellIndex
                ) {
                  // Swap into the previous cell
                  const occupiedImg = otherFolderImages.find(
                    (img) => img.id === occupiedById,
                  );
                  if (occupiedImg) {
                    ...
                    const swapFitScale = Math.min(
                      imageMaxSize / swapOrigW,
                      imageMaxHeight / swapOrigH,
                      1,
                    );
                    const swapDisplayW = swapOrigW * swapFitScale;
                    const swapOffsetX = (imageMaxSize - swapDisplayW) / 2;
                    swapX =
                      contentStartX + prevCell.col * CELL_SIZE + swapOffsetX;
              ...
              dragPrevCellRef.current = {
                ...
                col: finalCol,
                row: finalRow,
                cellIndex: finalRow * cols + finalCol,
              };
            } else {
            ...
            if (user) {
              ...
              if (lastSwappedImageRef.current) {
                ...
                const swappedImg = latestImages.find(
                  (img) => img.id === swappedRef.id,
                );
                const swappedCanonical =
                  swappedImg?.storagePath || swappedImg?.originalStoragePath;
      ...
      if (type === "image") {
        ...
        setTexts((prev) =>
          prev.map((txt) =>
            txt.id === node.id() ? { ...txt, x: newX, y: newY } : txt,
          ),
        );
      }
    },
    ...
    try {
      ...
      if (user) {
        try {
          ...
          const query = supabase.from(folderTable).delete().eq("id", folder.id);
  ...
  const handleRecenterVertically = useCallback(async () => {
    ...
    for (const folder of folders) {
      const h =
        folder.height ?? getFolderBorderHeight(folder, folder.imageIds.length);
    ...
    for (const folder of sortedFolders) {
      ...
      const newFolder = {
        ...folder,
        x: viewportCenterX - folder.width / 2,
        y: currentY,
      };
      recenteredFolders.push(newFolder);
      const deltaX = newFolder.x - folder.x;
  ...
  const handleImageDoubleClick = useCallback(
    (image: CanvasImage, e?: Konva.KonvaEventObject<MouseEvent>) => {
      if (e != null && e.evt.button !== 0) return;
      if (
        prevMouseDownButtonRef.current !== 0 ||
  ...
  const handleStageMouseDownWithZoom = useCallback(
    (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    <div className="relative h-full w-full bg-[#0d0d0d]">
      ...
      {applyPresetProgress && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-3 bg-[#171717] border border-[#2a2a2a] rounded-xl px-4 py-3 shadow-2xl shadow-black/50">
          <div className="w-5 h-5 border-2 border-[#3ECF8E] border-t-transparent rounded-full animate-spin" />
          <span className="text-white text-sm font-medium">
            Applying preset {applyPresetProgress.current} of{" "}
            {applyPresetProgress.total}
          </span>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
            <h2 className="text-lg font-semibold text-white mb-1">
              Add {pendingFileCount} photo{pendingFileCount > 1 ? "s" : ""}
            </h2>
            ...
            <input
              ...
              className={`w-full px-4 py-3 text-white bg-[#252525] border rounded-xl focus:outline-none transition-colors mb-1 ${
                folderNameError
                  ...
                  : "border-[#333] focus:border-[#3ECF8E] focus:ring-1 focus:ring-[#3ECF8E]/20"
              }`}
              onKeyDown={(e) => {
            ...
            <div className="flex gap-3">
              ...
              {selectedExistingFolderId ? (
                <button
                  onClick={() =>
                    addFilesToExistingFolder(selectedExistingFolderId)
                  }
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
            ...
            <p className="text-sm text-[#666] mb-4">
              {editingFolder.imageIds.length} photo
              {editingFolder.imageIds.length !== 1 ? "s" : ""} in this folder
            </p>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={
                    ...
                    editingFolderName === editingFolder.name
                  }
                  className="flex-1 px-4 py-2.5 text-sm font-medium text-[#0d0d0d] bg-[#3ECF8E] hover:bg-[#35b87d] disabled:bg-[#333] disabled:text-[#666] disabled:cursor-not-allowed rounded-xl transition-colors cursor-pointer"
                >
                  Save Name
              ...
              <div className="pt-3 border-t border-[#333]">
                  ...
                  onClick={() => {
                    ...
                    if (
                      ...
                      window.localStorage.getItem(
                        "driftboard-delete-folder-skip-confirm",
                      ) === "true"
                    ) {
                      handleDeleteFolder();
                    } else {
                ...
                >
                  {editingFolder.imageIds.length > 0
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
            <h2 className="text-lg font-semibold text-white mb-2">
              {confirmDeletePhotoIds.length === 1
                ? "Delete photo"
                : `Delete ${confirmDeletePhotoIds.length} photos`}
            </h2>
            <p className="text-sm text-[#888] mb-4">
              Are you sure? This will permanently delete{" "}
              {confirmDeletePhotoIds.length === 1
                ? "this photo"
                : `these ${confirmDeletePhotoIds.length} photos`}
              .
            </p>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  if (
                    deletePhotoDontAskAgain &&
                    typeof window !== "undefined"
                  ) {
                    window.localStorage.setItem(
                      "driftboard-delete-photo-skip-confirm",
                      "true",
                    );
                  }
      ...
      {confirmDeleteFolderOpen && editingFolder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
            ...
            <p className="text-sm text-[#888] mb-4">
              Are you sure? This will permanently delete the folder &quot;
              {editingFolder.name}&quot; and all {editingFolder.imageIds.length}{" "}
              photo{editingFolder.imageIds.length !== 1 ? "s" : ""} inside it.
            </p>
            <label className="flex items-center gap-2 mb-4 cursor-pointer">
              <input
                type="checkbox"
      ...
      {createSocialLayoutOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
            ...
            <p className="text-sm text-[#888] mb-4">
              Add a 4:5 layout. You can drag photos in and place them anywhere;
            ...
            <label className="block text-xs uppercase tracking-wide text-[#666] mt-4 mb-2">
              Number of pages (1–10)
            </label>
            <select
              value={createSocialLayoutPages}
              onChange={(e) =>
                setCreateSocialLayoutPages(Number(e.target.value))
              }
        ...
        createFolderFromSelectionIds.length > 0 && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
              ...
              <p className="text-sm text-[#888] mb-4">
                Name your folder ({createFolderFromSelectionIds.length} photo
                {createFolderFromSelectionIds.length !== 1 ? "s" : ""}{" "}
                selected). Existing folders will be pushed aside if needed.
              </p>
              <label className="block text-xs uppercase tracking-wide text-[#666] mb-2">
                Folder name
              </label>
      ...
      >
        ...
        >
          <Layer>
              ...
              folders
                .filter(
                  (folder) =>
                    ...
                    folder.imageIds.some((id) => visibleImageIds.has(id)),
                )
                .map((folder) => {
                    ...
                    <Group
                      ...
                      onTouchMove={
                        isMobile
                          ? (e) => {
                              const touch = e.evt.touches[0];
                              if (!touch || !longPressTouchPosRef.current)
                                return;
                              const dx =
                                touch.clientX - longPressTouchPosRef.current.x;
                              const dy =
                                touch.clientY - longPressTouchPosRef.current.y;
                              if (Math.sqrt(dx * dx + dy * dy) > 10)
                                cancelLongPress();
                            }
                          : undefined
                      ...
                      onTouchEnd={
                        isMobile ? () => cancelLongPress() : undefined
                      }
                    >
                      ...
                      <Group
                        ...
                        onDragMove={(e) => {
                          ...
                          const latestFolders =
                            useCanvasStore.getState().folders;
                          ...
                          if (folderImgs.length > 0) {
                            ...
                            updatedImages = latestImages.map((img) => {
                              const reflowed = reflowedImages.find(
                                (r) => r.id === img.id,
                              );
                        ...
                        <Text
                          ref={(el) => {
                            if (el) {
                              ...
                              requestAnimationFrame(() => {
                                const w = el.width();
                                setFolderLabelWidths((prev) =>
                                  prev[currentFolder.id] === w
                                    ? prev
                                    : { ...prev, [currentFolder.id]: w },
                                );
                              });
                      ...
                      {isSocialLayout(currentFolder) ? (
                        (() => {
                          const n = Math.max(
                            ...
                            Math.min(
                              SOCIAL_LAYOUT_MAX_PAGES,
                              currentFolder.pageCount ?? 1,
                            ),
                          );
                          const pageW = SOCIAL_LAYOUT_PAGE_WIDTH;
                          const bg =
                            currentFolder.backgroundColor ??
                            DEFAULT_SOCIAL_LAYOUT_BG;
                        ...
                        strokeWidth={
                          dragSourceFolderBorderHovered === currentFolder.id
                            ? dragBorderBlink
                              ? 3
                              : 2
                            : dragHoveredFolderId === currentFolder.id ||
                                isHovered
                              ? 3
                              : 1
                        }
                      ...
                      {!isSocialLayout(currentFolder) && (
                        <Rect
                          ...
                          onDragMove={(e) => {
                            ...
                            const oldCols = calculateColsFromWidth(
                              currentFolder.width,
                            );
                            const newCols = calculateColsFromWidth(newWidth);
                            const dimensionsChanged =
                              newWidth !== currentFolder.width ||
                              newHeight !== currentFolder.height;
                            const needsRepack =
                              newCols !== oldCols ||
                              newHeight < (currentFolder.height ?? Infinity);
                            ...
                            if (folderImgs.length > 0) {
                              if (dimensionsChanged && needsRepack) {
                                ...
                                cellAssignments = currentPositions.map(
                                  (pos) => ({
                                    imageId: pos.imageId,
                                    col: pos.col,
                                    row: pos.row,
                            ...
                            if (
                              ...
                              updatedImages = images.map((img) => {
                                const positioned = positionedImages.find(
                                  (p) => p.id === img.id,
                                );
                          ...
                          onDragEnd={async (e) => {
                            ...
                            const resizedFolder = stateAfterFlush.folders.find(
                              (f) => f.id === currentFolder.id,
                            );
                            if (!resizedFolder) return;

                            // Get folder images and their current positions
                            const folderImgs = stateAfterFlush.images.filter(
                              (img) => resizedFolder.imageIds.includes(img.id),
                            );
                            ...
                            const maxRowWithImage =
                              imageCount > 0
                                ? Math.max(
                                    0,
                                    ...currentPositions.map((p) => p.row),
                                  )
                                : 0;
                            ...
                            const snappedContentHeight =
                              ...
                              GRID_CONFIG.imageGap;
                            const snappedHeight =
                              30 + Math.max(snappedContentHeight, 100);

                            // Check if snapped dimensions would cut off any images
                            const snappedCols =
                              calculateColsFromWidth(snappedWidth);
                            ...
                            let cellAssignments: CellAssignment[];
                            const needsRepack = currentPositions.some(
                              (p) =>
                                p.col >= snappedCols || p.row >= snappedMaxRows,
                            );
                            ...
                            const snappedFolders = stateAfterFlush.folders.map(
                              (f) =>
                            ...
                            const snappedImages = stateAfterFlush.images.map(
                              (img) => {
                                const positioned = positionedImages.find(
                                  (p) => p.id === img.id,
                                );
                      ...
                      {dragGhostPosition &&
                        dragGhostPosition.folderId === currentFolder.id && (
                          <Rect
            ...
            {(visibleImageIds === null
              ? images
              : images.filter((img) => visibleImageIds.has(img.id))
            )
              .filter(
                (img) =>
                  viewportVisibleIds.has(img.id) ||
            ...
            {images
              .filter(
                (img) =>
                  selectedIds.includes(img.id) &&
                  (visibleImageIds === null || visibleImageIds.has(img.id)),
              )
              .map((img) => {
                const folder = folders.find(
                  (f) => f.id === img.folderId || f.imageIds.includes(img.id),
                );
                const strokeColor = folder
                  ? hexToRgba(folder.color, 0.4)
                  : hexToRgba("#3ECF8E", 0.4);
                ...
                if (img.folderId) {
                  const { imageMaxSize, imageMaxHeight } = GRID_CONFIG;
                  const fitScale = Math.min(
                    imageMaxSize / displayW,
                    imageMaxHeight / displayH,
                    1,
                  );
                  displayW *= fitScale;
                  displayH *= fitScale;
      ...
      {selectedFolderId &&
        (() => {
          ...
          const pageCount = Math.max(
            1,
            Math.min(SOCIAL_LAYOUT_MAX_PAGES, selectedFolder.pageCount ?? 1),
          );
        ...
        (() => {
          ...
          return (
            ...
            >
              {isLayout && (
                <>
                  <div className="px-4 py-2 text-xs text-[#666] uppercase tracking-wide">
                    Layout
                  </div>
          ...
          {imageContextMenu.selectedIds.length > 1 ? (
            <>
              <button
                ...
                className="w-full px-4 py-2.5 text-left text-sm text-white hover:bg-[#252525] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Paste edit to selection ({imageContextMenu.selectedIds.length}{" "}
                photos)
              </button>
              {user && (
                <button
      ...
      {createPresetFromImageId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96">
            ...
            <p className="text-sm text-[#888] mb-4">
              Save this image’s edits as a preset you can apply to other photos.
            </p>
            <input
              type="text"
      ...
      {applyPresetToSelectionIds && applyPresetToSelectionIds.length > 0 && (
        ...
        >
          <div
            className="bg-[#171717] border border-[#2a2a2a] rounded-2xl shadow-2xl shadow-black/50 p-6 w-96 max-h-[80vh] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold text-white mb-2">
              Apply preset
            ...
            <p className="text-sm text-[#888] mb-4">
              Choose a preset to apply to {applyPresetToSelectionIds.length}{" "}
              photo{applyPresetToSelectionIds.length !== 1 ? "s" : ""}
            </p>
      ...
      {selectedObject && (!isMobile || mobileEditFullscreen) && (
        <EditPanel
          ...
          imagePreviewUrl={
            isMobile && "src" in selectedObject
              ? (selectedObject as CanvasImage).src
              : undefined
          }
          onUpdate={(updates) => {
            if ("src" in selectedObject) {
              ...
              if (imageId) {
                ...
                const debounceOk =
                  now - lastEditHistoryPushRef.current >=
                ...
                if (isBulkUpdate || debounceOk) {
                  ...
                  setEditHistory((prev) => [
                    ...
                    {
                      imageId,
                      snapshot: getEditSnapshot(selectedObject as CanvasImage),
                    },
                  ]);
                  setEditRedoStack([]);
                }
              ...
              startTransition(() => {
                setImages((prev) =>
                  prev.map((img) =>
                    img.id === selectedIds[0] ? { ...img, ...updates } : img,
                  ),
                );
              });
            } else {
              setTexts((prev) =>
                prev.map((txt) =>
                  txt.id === selectedIds[0] ? { ...txt, ...updates } : txt,
                ),
              );
            }
          }}
          onDelete={async () => {
            if ("src" in selectedObject) {
              ...
              setTexts((prev) =>
                prev.filter((txt) => txt.id !== selectedIds[0]),
              );
              setSelectedIds([]);
              saveToHistory();
            }
          ...
          onRetrySave={
            "src" in selectedObject ? () => handleSave(false) : undefined
          }
