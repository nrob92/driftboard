...
function Slider({
  ...
  const settledRef = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
...
export function EditPanel(props: EditPanelProps) {
  ...
  const updatePresets = useCallback(
    (updater: (prev: Preset[]) => Preset[]) => {
      queryClient.setQueryData<Preset[]>(presetsQueryKey, (prev) =>
        updater(prev ?? []),
      );
    },
  ...
  const isColorModified =
    isImage &&
    (img.temperature !== 0 || img.vibrance !== 0 || img.saturation !== 0);
  ...
  const parseXMP = (xmpContent: string): Partial<CanvasImage> => {
    ...
    if (temperature !== null) {
      ...
      settings.temperature = Math.max(
        -1,
        Math.min(1, (5500 - temperature) / 3500),
      );
    }
    ...
    const colors: Array<keyof ColorHSL> = [
      "red",
      "orange",
      "yellow",
      "green",
    ...
    if (
      colorGradeShadowLum !== null ||
      colorGradeMidtoneLum !== null ||
      colorGradeHighlightLum !== null ||
      colorGradeMidtoneHue !== null ||
      colorGradeMidtoneSat !== null ||
      colorGradeGlobalHue !== null ||
      colorGradeGlobalSat !== null ||
      colorGradeGlobalLum !== null ||
      colorGradeBlending !== null
    ) {
    ...
    const extractCurve = (channelName: string): CurvePoint[] => {
      ...
      const tagName = channelName
        ? `ToneCurvePV2012${channelName}`
        : "ToneCurvePV2012";
      const pattern = new RegExp(
        `<crs:${tagName}>\\s*<rdf:Seq>([\\s\\S]*?)<\\/rdf:Seq>\\s*<\\/crs:${tagName}>`,
        "i",
      );
  ...
  const handlePresetFiles = useCallback(
  ...
  const handleRenameSubmit = useCallback(async () => {
    ...
    updatePresets((prev) =>
      prev.map((p) =>
        p.id === renamingPresetId ? { ...p, name: newName } : p,
      ),
    );
  ...
  if (isMobile && isImage) {
    ...
    return (
      <div
        className="fixed inset-0 z-40 bg-[#0d0d0d] flex flex-col"
        style={{ height: "100dvh" }}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 touch-pan-y">
                        ...
                        return (
                          <div key={color} className="mb-4">
                            <div
                              ...
                              style={{ color: colorLabels[color] }}
                            >
                              {color.charAt(0).toUpperCase() + color.slice(1)}
                            </div>
                            <Slider
                              label="Hue"
                              value={img.colorHSL?.[color]?.hue ?? 0}
                              min={-100}
                              max={100}
                              step={1}
                              defaultValue={0}
                                ...
                                onUpdate({
                                  colorHSL: {
                                    ...
                                    [color]: {
                                      ...
                                      hue: v,
                                      saturation:
                                        img.colorHSL?.[color]?.saturation ?? 0,
                                      luminance:
                                        img.colorHSL?.[color]?.luminance ?? 0,
                                    },
                                  } as ColorHSL,
                                })
                              }
                            ...
                            <Slider
                              label="Saturation"
                              value={img.colorHSL?.[color]?.saturation ?? 0}
                              min={-100}
                              max={100}
                              step={1}
                              defaultValue={0}
                            ...
                            <Slider
                              label="Luminance"
                              value={img.colorHSL?.[color]?.luminance ?? 0}
                              min={-100}
                              max={100}
                              step={1}
                              defaultValue={0}
          ...
          {activePanel === "effects" && (
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    onUpdate({ clarity: 0, dehaze: 0, vignette: 0, grain: 0 })
                  }
                  className="text-xs text-[#888] hover:text-white transition-colors"
                >
                  Reset
          ...
          {activePanel === "presets" && (
            <div>
              ...
              >
                <svg
                  className="w-6 h-6 mx-auto mb-1 text-[#666]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                {presets.length === 0 ? (
                  ...
                  presets
                    ...
                    .map((preset) => (
                      <div
                        key={preset.id}
                        className="flex items-center justify-between gap-2 p-3 min-h-[44px] bg-[#252525] rounded-lg"
                      >
                        ...
                        <button
                          ...
                          className="p-2 text-[#888] hover:text-[#f87171] transition-colors flex-shrink-0"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
        ...
        {/* Bottom Action Bar */}
        <div
          className="flex-shrink-0 flex items-center justify-around px-4 py-3 border-t border-[#2a2a2a] bg-[#171717]"
          style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}
        >
          {onResetToOriginal && (
            <button
              onClick={onResetToOriginal}
              className="flex flex-col items-center gap-1 p-2 min-h-[44px] min-w-[44px] text-[#999] rounded-lg transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="none"
          ...
          {onExport && (
            <button
              onClick={onExport}
              className="flex flex-col items-center gap-1 p-2 min-h-[44px] min-w-[44px] text-[#6366f1] rounded-lg transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="none"
              ...
              {isImage ? (
                <>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-medium uppercase tracking-wider ${bypassedTabs?.has("curves") ? "line-through" : ""}`}
                      >
                        Curves
                      </span>
                    </button>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-medium uppercase tracking-wider ${bypassedTabs?.has("light") ? "line-through" : ""}`}
                      >
                        Light
                      </span>
                    </button>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-medium uppercase tracking-wider ${bypassedTabs?.has("color") ? "line-through" : ""}`}
                      >
                        Color
                      </span>
                    </button>
                      ...
                      <div className={modalWrapper}>
                        <div className="bg-[#171717] border border-[#2a2a2a] rounded-xl shadow-2xl shadow-black/50 p-4 w-80">
                          ...
                          <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
                            <div className="flex items-center justify-between gap-3">
                              <span className="text-xs text-[#888] w-20">
                                Temp
                              </span>
                              <div className="flex items-center gap-2 flex-1">
                                <span className="text-[10px] text-[#74c0fc]">
                                  Cool
                                </span>
                                <input
                                  type="range"
                                ...
                                />
                                <span className="text-[10px] text-[#ff9f43]">
                                  Warm
                                </span>
                              </div>
                            </div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-medium uppercase tracking-wider ${bypassedTabs?.has("effects") ? "line-through" : ""}`}
                      >
                        Effects
                      </span>
                    </button>
                      ...
                      <div className={modalWrapper}>
                        <div className="bg-[#171717] border border-[#2a2a2a] rounded-xl shadow-2xl shadow-black/50 p-4 w-80">
                          ...
                          <div
                            ...
                            onDrop={handleDrop}
                            onClick={() => fileInputRef.current?.click()}
                            className={`mb-3 border-2 border-dashed rounded-lg py-3 px-4 text-center cursor-pointer transition-colors ${isDraggingOver ? "border-[#3ECF8E] bg-[#3ECF8E]/10" : "border-[#333] bg-[#252525] hover:border-[#3ECF8E]/50"}`}
                          >
                            <svg
                              className="w-6 h-6 mx-auto mb-1 text-[#666]"
                              fill="none"
                            ...
                            {presets.length === 0 ? (
                              ...
                              presets
                                ...
                                .map((preset) => (
                                  <div
                                    key={preset.id}
                                    className="flex items-center justify-between gap-2 p-2 rounded-lg bg-[#252525] hover:bg-[#2a2a2a]"
                                  >
                                    {renamingPresetId === preset.id ? (
                                      <input
                                        type="text"
                                      ...
                                      <button
                                        ...
                                        onDoubleClick={(e) => {
                                          e.preventDefault();
                                          handlePresetDoubleClick(preset);
                                        }}
                                        className="flex-1 text-left text-sm text-white hover:text-[#3ECF8E]"
                                      >
                                        {preset.name}
                                      </button>
                                    )}
                                    <button
                                      onClick={() => deletePreset(preset.id)}
                                      className="p-1 text-[#888] hover:text-[#f87171] transition-colors"
                                      title="Delete preset"
                                    >
                                      <svg
                                        className="w-4 h-4"
                ...
                <>
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-medium text-[#888] uppercase tracking-wider">
                      Text
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-xs text-[#666] w-6 tabular-nums">
                        {(object as CanvasText).fontSize}
                      </span>
                    </div>
                  </div>
