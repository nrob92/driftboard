...
function getFolderBorderHeight(
  imageCount: number,
  folderWidth: number,
  folderHeight?: number,
): number {
  ...
  if (layoutMode === "stack") {
    ...
    return images.map((img, index) => {
      ...
      const scale = Math.min(
        imageMaxSize / baseWidth,
        imageMaxSize / baseHeight,
        1,
      );
...
function SandboxImageNode({
  ...
  const filterList = useMemo(
    () => buildExportFilterList(toCanvasImage(img, DEFAULT_CURVES)),
    [img],
  );
...
export function LoginSandbox({
  ...
  const [activePanel, setActivePanel] = useState<
    "curves" | "light" | "color" | "effects" | null
  >(null);
  ...
  >({});
  const [hoveredFolderBorder, setHoveredFolderBorder] = useState<string | null>(
    null,
  );
    ...
    (
      ...
      const newImages: SandboxImage[] = toAdd.map((item, i) => ({
        ...
        x: addToFolder
          ...
          : 100 +
            ((startIndex + i) % 3) *
              (GRID_CONFIG.imageMaxSize + GRID_CONFIG.imageGap),
        y: addToFolder
          ? 0
          : 100 +
            Math.floor((startIndex + i) / 3) *
              (GRID_CONFIG.imageMaxSize + GRID_CONFIG.imageGap),
        width: item.width,
        height: item.height,
        ...
        setSandboxFolders((prev) =>
          prev.map((f) =>
            f.id === targetFolderId
              ? {
                  ...f,
                  imageIds: [...f.imageIds, ...newImages.map((im) => im.id)],
                }
              : f,
          ),
        );
      ...
      setSandboxImages((prev) => {
        ...
        const targetFolder: SandboxFolder =
          (targetFolderId
            ? sandboxFolders.find((f) => f.id === targetFolderId)
            : null) ?? newFolder;
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const targetFolderId =
        sandboxFolders.length === 1 ? sandboxFolders[0].id : undefined;
      let loaded = 0;
  ...
  const handleImageDragMove = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>) => {
      ...
      for (const folder of sandboxFolders) {
        const contentHeight = getFolderBorderHeight(
        ...
        if (
          currentCenterX >= left &&
          currentCenterX <= right &&
          currentCenterY >= top &&
          currentCenterY <= bottom
        ) {
      ...
      if (targetFolderId && targetFolder) {
        ...
        const targetRow = Math.max(
          0,
          Math.min(maxRows - 1, Math.floor(relativeY / CELL_SIZE)),
        );

        // Get other images in folder
        const otherFolderImages = sandboxImages.filter(
          (img) =>
        ...
        if (occupiedCells.has(targetCellKey)) {
          if (isInValidSpot) {
            ...
            if (occupiedImg && !occupiedCells.has(currentCellKey)) {
              ...
              const baseHeight =
                occupiedImg.originalHeight ?? occupiedImg.height;
              const scale = Math.min(
                imageMaxSize / baseWidth,
                imageMaxSize / baseHeight,
                1,
    ...
    (id: string, x: number, y: number) => {
      ...
      if (targetFolderId !== oldFolderId) {
        if (!targetFolderId && oldFolderId) {
          ...
          const untitledNames = sandboxFolders
          ...
          const updatedOldFolder = {
            ...oldFolder,
            imageIds: oldFolder.imageIds.filter((i) => i !== id),
          };

          // Get remaining images in old folder
          const remainingOldFolderImages = sandboxImages.filter(
            (i) => i.folderId === oldFolderId && i.id !== id,
          );
          ...
          const updatedImages = sandboxImages.map((i) => {
            ...
            if (i.folderId === oldFolderId) {
              const reflowed = reflowedOldFolderImages.find(
                (r) => r.id === i.id,
              );
              if (reflowed) {
                return {
        ...
        if (targetFolderId && targetFolder) {
          ...
          const contentStartX =
            currentTargetFolder.x + GRID_CONFIG.folderPadding;
          const contentStartY =
            currentTargetFolder.y + LABEL_HEIGHT + GRID_CONFIG.folderPadding;
          ...
          if (currentTargetFolder.height != null) {
            ...
            maxRows = Math.max(
              1,
              Math.floor(availableContentHeight / CELL_SIZE),
            );
          }
          ...
          const otherFolderImages = sandboxImages.filter(
            (img) => img.folderId === targetFolderId && img.id !== id,
          );
          ...
          if (oldFolderId) {
            ...
            if (oldFolder) {
              ...
              const oldContentStartY =
                oldFolder.y + LABEL_HEIGHT + GRID_CONFIG.folderPadding;
          ...
          if (occupiedCells.has(targetCellKey)) {
            if (isInValidSpot && oldFolderId === targetFolderId) {
              ...
              if (occupiedImg && !occupiedCells.has(currentCellKey)) {
                const baseWidth =
                  occupiedImg.originalWidth ?? occupiedImg.width;
                const baseHeight =
                  occupiedImg.originalHeight ?? occupiedImg.height;
                const scale = Math.min(
                  GRID_CONFIG.imageMaxSize / baseWidth,
                  GRID_CONFIG.imageMaxSize / baseHeight,
                  1,
                );
                const swapImgWidth = baseWidth * scale;
                const swapImgHeight = baseHeight * scale;
                const swapOffsetX =
                  (GRID_CONFIG.imageMaxSize - swapImgWidth) / 2;
                const swapOffsetY =
                  (GRID_CONFIG.imageMaxSize - swapImgHeight) / 2;
          ...
          const updatedFolders = sandboxFolders
            .map((f) => {
              if (f.id === oldFolderId)
                return { ...f, imageIds: f.imageIds.filter((i) => i !== id) };
              if (f.id === targetFolderId)
                return { ...f, imageIds: [...f.imageIds, id] };
              return f;
            })
            .filter((f) => f.imageIds.length > 0);
          ...
          const oldFolder = sandboxFolders.find((f) => f.id === oldFolderId);
          const remainingOldFolderImages = sandboxImages.filter(
            (im) => im.folderId === oldFolderId && im.id !== id,
          );
          let reflowedOldFolderImages: SandboxImage[] = [];
          if (
            oldFolder &&
            remainingOldFolderImages.length > 0 &&
          ...
          const targetFolderImages = sandboxImages.filter(
            (im) => im.folderId === targetFolderId || im.id === id,
          );
          ...
          const updatedImages = sandboxImages.map((i) => {
            if (i.id === id) {
              ...
              const reflowed = reflowedTargetFolderImages.find(
                (r) => r.id === id,
              );
              if (reflowed) {
                return {
      ...
      if (targetFolderId && targetFolder && targetFolderId === oldFolderId) {
        ...
        const contentStartY =
          targetFolder.y + LABEL_HEIGHT + GRID_CONFIG.folderPadding;
  ...
  const updateImageEdit = useCallback(
    (id: string, updates: Partial<SandboxImage>) => {
      setSandboxImages((prev) =>
  ...
  const handleDeleteImage = useCallback(() => {
    ...
    setSandboxFolders((prev) =>
      prev
        .map((f) => ({
          ...f,
          imageIds: f.imageIds.filter((id) => id !== selectedId),
        }))
        .filter((f) => f.imageIds.length > 0),
    );
    setSelectedId(null);
  ...
  const isCurvesModified =
    selectedImage?.curves &&
    (isCurveModified(selectedImage.curves.rgb) ||
      isCurveModified(selectedImage.curves.red) ||
      isCurveModified(selectedImage.curves.green) ||
      isCurveModified(selectedImage.curves.blue));
  const isLightModified =
    selectedImage &&
    ((selectedImage.exposure ?? 0) !== 0 ||
      (selectedImage.contrast ?? 0) !== 0 ||
      (selectedImage.highlights ?? 0) !== 0 ||
      (selectedImage.shadows ?? 0) !== 0 ||
      (selectedImage.whites ?? 0) !== 0 ||
      (selectedImage.blacks ?? 0) !== 0);
  const isColorModified =
    selectedImage &&
    ((selectedImage.temperature ?? 0) !== 0 ||
      (selectedImage.vibrance ?? 0) !== 0 ||
      (selectedImage.saturation ?? 0) !== 0);
  const isEffectsModified =
    selectedImage &&
    ((selectedImage.clarity ?? 0) !== 0 ||
      (selectedImage.dehaze ?? 0) !== 0 ||
      (selectedImage.vignette ?? 0) !== 0 ||
      (selectedImage.grain ?? 0) !== 0);
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto">
                ...
                <Group key={folder.id}>
                  ...
                  <Group
                    ...
                    onDragMove={(e) => {
                      ...
                      const folderImgs = sandboxImages.filter((img) =>
                  ...
                  <Rect
                    ...
                    height={20}
                    fill={
                      isHovered || resizingFolderId === folder.id
                        ? folder.color
                        : "transparent"
                    }
                    opacity={
                      isHovered || resizingFolderId === folder.id ? 0.6 : 0
                    }
                    cornerRadius={4}
                    draggable={true}
                    ...
                    onDragMove={(e) => {
                      const handleSize = 20;
                      const currentFolder =
                        sandboxFolders.find((f) => f.id === folder.id) ||
                        folder;
                      const proposedWidth = Math.max(
                        GRID_CONFIG.minFolderWidth,
                        e.target.x() - currentFolder.x + handleSize,
                      );
                      ...
                      const proposedHeight =
                        LABEL_HEIGHT + proposedContentHeight;
                      ...
                      const proposedCols =
                        calculateColsFromWidth(proposedWidth);

                      // Calculate minimum size needed for all images
                      const rowsNeeded =
                        Math.ceil(folderImgs.length / proposedCols) || 1;
                      const minContentHeight =
                        rowsNeeded * CELL_SIZE + 2 * GRID_CONFIG.folderPadding;
                      const minHeight =
                        LABEL_HEIGHT + Math.max(minContentHeight, 100);

                      // Snap width to grid (exact fit for columns with proper padding)
                      const snappedWidth =
                        2 * GRID_CONFIG.folderPadding +
                        proposedCols * CELL_SIZE -
                        GRID_CONFIG.imageGap;

                      // Snap height to grid (based on rows needed)
                      const snappedContentHeight =
                        2 * GRID_CONFIG.folderPadding +
                        rowsNeeded * CELL_SIZE -
                        GRID_CONFIG.imageGap;
                      const snappedHeight =
                        LABEL_HEIGHT + Math.max(snappedContentHeight, 100);
                      ...
                      e.target.y(
                        borderY + (newHeight - LABEL_HEIGHT) - handleSize,
                      );
                    ...
                    onDragEnd={(e) => {
                      ...
                      const resizedFolder = sandboxFolders.find(
                        (f) => f.id === folder.id,
                      );
                      if (resizedFolder) {
                        const folderImgs = sandboxImages.filter((img) =>
                          resizedFolder.imageIds.includes(img.id),
                        );
                        ...
                        const rowsNeeded =
                          Math.ceil(folderImgs.length / cols) || 1;
      ...
      {selectedImage && selectedId && (
        <div
          className="absolute left-1/2 -translate-x-1/2 z-10 mt-3"
          style={{ top: "540px" }}
        >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() =>
                      setActivePanel(activePanel === "curves" ? null : "curves")
                    }
                  ...
                  {activePanel === "curves" && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 z-20 mb-2">
                      <CurvesEditor
                        ...
                        onChange={(curves) =>
                          updateImageEdit(selectedId, { curves })
                        }
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() =>
                      setActivePanel(activePanel === "light" ? null : "light")
                    }
                    className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg transition-all duration-150 cursor-pointer ${
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 z-20 mb-2">
                      <div className="bg-[#171717] border border-[#2a2a2a] rounded-xl shadow-2xl shadow-black/50 p-2.5 min-w-[256px] max-w-fit">
                        ...
                        <div className="space-y-2">
                          <SandboxSlider
                            label="Exposure"
                            value={selectedImage.exposure ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { exposure: v })
                            }
                          />
                          <SandboxSlider
                            label="Contrast"
                            value={selectedImage.contrast ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { contrast: v })
                            }
                          />
                          <SandboxSlider
                            label="Highlights"
                            ...
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { highlights: v })
                            }
                          />
                          <SandboxSlider
                            label="Shadows"
                            ...
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { shadows: v })
                            }
                          />
                          <SandboxSlider
                            label="Whites"
                            value={selectedImage.whites ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { whites: v })
                            }
                          />
                          <SandboxSlider
                            label="Blacks"
                            value={selectedImage.blacks ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { blacks: v })
                            }
                          />
                        </div>
                      </div>
                      ...
                      <div className="bg-[#171717] border border-[#2a2a2a] rounded-xl shadow-2xl shadow-black/50 p-2.5 min-w-[256px] max-w-fit">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-2.5">
                            <span className="text-[10px] text-[#888] w-18">
                              Temp
                            </span>
                            <div className="flex items-center gap-2 flex-1">
                              <span className="text-[9px] text-[#74c0fc]">
                                Cool
                              </span>
                              <input
                                type="range"
                              ...
                              />
                              <span className="text-[9px] text-[#ff9f43]">
                                Warm
                              </span>
                            </div>
                          </div>
                          <SandboxSlider
                            label="Vibrance"
                            value={selectedImage.vibrance ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { vibrance: v })
                            }
                          />
                          <SandboxSlider
                            label="Saturation"
                            value={selectedImage.saturation ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { saturation: v })
                            }
                          />
                        </div>
                      </div>
                ...
                <div className="relative">
                  <button
                    ...
                    onClick={() =>
                      setActivePanel(
                        activePanel === "effects" ? null : "effects",
                      )
                    }
                    className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg transition-all duration-150 cursor-pointer ${
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 z-20 mb-2">
                      <div className="bg-[#171717] border border-[#2a2a2a] rounded-xl shadow-2xl shadow-black/50 p-3 min-w-[256px] max-w-fit">
                        ...
                        <div className="space-y-2.5">
                          <SandboxSlider
                            label="Clarity"
                            value={selectedImage.clarity ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { clarity: v })
                            }
                          />
                          <SandboxSlider
                            label="Dehaze"
                            value={selectedImage.dehaze ?? 0}
                            min={-1}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { dehaze: v })
                            }
                          />
                          <SandboxSlider
                            label="Vignette"
                            value={selectedImage.vignette ?? 0}
                            min={0}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { vignette: v })
                            }
                          />
                          <SandboxSlider
                            label="Grain"
                            value={selectedImage.grain ?? 0}
                            min={0}
                            max={1}
                            step={0.01}
                            defaultValue={0}
                            onChange={(v) =>
                              updateImageEdit(selectedId, { grain: v })
                            }
                          />
                        </div>
                      </div>
