...
export function useCollaboration({ config, onPhotosUpdate, onFoldersUpdate }: UseCollaborationProps) {
    ...
    channel
      ...
      .on('broadcast', { event: 'cursor' }, ({ payload }) => {
        setOnlineUsers(prev => prev.map(u => 
          u.userId === payload.userId ? { ...u, x: payload.x, y: payload.y } : u
        ));
      })
  ...
  const broadcastActivity = useCallback((action: string, targetType: string, targetId: string, metadata: Record<string, unknown> = {}) => {
    if (!config || !channelRef.current) return;
  ...
  const broadcastPhotosUpdate = useCallback((photos: CanvasImage[]) => {
    if (!config || !channelRef.current) return;
    channelRef.current.send({
      type: 'broadcast',
      event: 'photo_update',
  ...
  const broadcastFoldersUpdate = useCallback((folders: PhotoFolder[]) => {
    if (!config || !channelRef.current) return;
    channelRef.current.send({
      type: 'broadcast',
      event: 'folder_update',
...
function getUserColor(index: number): string {
  const colors = ['#3ECF8E', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#06B6D4'];
