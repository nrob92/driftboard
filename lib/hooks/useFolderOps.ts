...
import {
  ...
  DEFAULT_SOCIAL_LAYOUT_BG, FOLDER_COLORS, GRID_CONFIG,
  isSocialLayout, reflowImagesInFolder, getFolderBounds,
...
export function useFolderOps({
  ...
  const handleCreateFolderFromSelectionSave = useCallback(async () => {
    const ui = useUIStore.getState();
    const { images, folders, setFolders, setImages } = useCanvasStore.getState();
    ...
    if (folders.some((f) => f.name.toLowerCase() === name.toLowerCase())) {
      ui.setCreateFolderFromSelectionNameError('A folder with this name already exists');
      return;
    }
    ...
    saveToHistory();
  }, [user, resolveOverlapsAndReflow, saveToHistory, queryClient, skipNextPhotosLoadRef]);
  ...
  const handleCreateEmptyFolderSave = useCallback(async () => {
    ...
    if (folders.some((f) => f.name.toLowerCase() === name.toLowerCase())) {
      ui.setCreateEmptyFolderNameError('A folder with this name already exists');
      return;
    }
  ...
  const handleCreateSocialLayoutSave = useCallback(async () => {
    ...
    if (folders.some((f) => f.name.toLowerCase() === name.toLowerCase())) {
      ui.setCreateSocialLayoutNameError('A folder or layout with this name already exists');
      return;
    }

    const pages = Math.max(1, Math.min(SOCIAL_LAYOUT_MAX_PAGES, ui.createSocialLayoutPages));
    const centerX = (dimensions.width / 2 - stagePosition.x) / stageScale;
    const centerY = (dimensions.height / 2 - stagePosition.y) / stageScale;
    const folderId = `folder-${Date.now()}`;
    const colorIndex = folders.length % FOLDER_COLORS.length;
  ...
  const handleDeleteFolder = useCallback(async () => {
    ...
    ui.setEditingFolderName('');
    ui.setFolderNameError('');
    ui.setDeleteFolderProgress(total > 0 ? { current: 0, total } : { current: 0, total: 0 });
  ...
  const handleRecenterFolders = useCallback(async () => {
    const { images, folders, setFolders, setImages, stagePosition, stageScale, dimensions } = useCanvasStore.getState();
