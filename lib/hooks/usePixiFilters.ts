...
import { getPixiFilterEngine, PixiFilterEngine } from '@/lib/filters/pixiFilterEngine';
...
export function usePixiFilters({
  ...
  const filterSignature = useMemo(() => {
    ...
    image.filters, image.width, image.height,
    bypassedTabs,
  ]);
  ...
  const hasActiveFilters = useMemo(() => {
    ...
    return image.exposure !== 0 || image.contrast !== 0 || image.highlights !== 0 ||
      ...
      image.blur > 0 || (image.filters?.length ?? 0) > 0 ||
      ...
      (image.curves && JSON.stringify(image.curves) !== JSON.stringify({
