...
interface CanvasState {
  ...
  // Actions - data
  setImages: (
    images: CanvasImage[] | ((prev: CanvasImage[]) => CanvasImage[]),
  ) => void;
  updateImage: (id: string, updates: Partial<CanvasImage>) => void;
  setTexts: (
    texts: CanvasText[] | ((prev: CanvasText[]) => CanvasText[]),
  ) => void;
  setFolders: (
    folders: PhotoFolder[] | ((prev: PhotoFolder[]) => PhotoFolder[]),
  ) => void;
...
export const useCanvasStore = create<CanvasState>()(
  immer((set, get) => ({
    ...
    setTexts: (textsOrFn) =>
      set((state) => {
        state.texts =
          typeof textsOrFn === "function" ? textsOrFn(state.texts) : textsOrFn;
      }),

    setFolders: (foldersOrFn) =>
      set((state) => {
        state.folders =
          typeof foldersOrFn === "function"
    ...
    setSelectedIds: (idsOrFn) =>
      set((state) => {
        state.selectedIds =
          typeof idsOrFn === "function" ? idsOrFn(state.selectedIds) : idsOrFn;
      }),
...
export const selectSelectedImage = (
  state: CanvasState,
